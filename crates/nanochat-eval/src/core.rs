...
/// * `data_dir` - Directory containing CORE benchmark data (core.jsonl)
/// * `batch_size` - Batch size for evaluation (currently unused, for future batching)
...
pub fn evaluate_core(
    ...
    let file = File::open(&jsonl_path)
        .map_err(|e| anyhow::anyhow!("Failed to open CORE data file: {}", e))?;
    ...
    metadata_map.insert("benchmark".to_string(), Value::String("CORE".to_string()));
    metadata_map.insert(
