...
//! nanochat-midtrain \
//!   --config config.json \
//!   --base-model ./checkpoints/pretrained.safetensors \
...
/// Mid-training stage for nanochat conversational fine-tuning
#[derive(Parser, Debug)]
#[command(name = "nanochat-midtrain")]
#[command(
    about = "Mid-training stage for nanochat conversational fine-tuning",
    long_about = None
)]
struct Args {
    ...
    /// Path to pretrained base model checkpoint (required, SafeTensors format)
    #[arg(long, value_name = "PATH", required = true)]
    base_model: PathBuf,

    /// Directory containing conversational training data (JSONL files)
    #[arg(long, value_name = "PATH", required = true)]
    data_dir: PathBuf,
...
fn main() -> Result<()> {
    ...
    // Load tokenizer (should match the one used for pretraining)
    ...
    if !args.quiet {
        println!("Mid-training completed successfully!");
    }
