...
use nanochat_model::{config::ConfigError, validate_tokenizer_model_compatibility, GPTConfig, GPT};
...
fn test_tokenizer_model_with_special_tokens() -> anyhow::Result<()> {
    // Create a tokenizer
    let corpus = ["hello world", "test message"];
    let tokenizer = Tokenizer::train_from_iterator(corpus.iter(), 500)?;
    ...
    if let (Ok(_bos_id), Ok(_eos_id)) = (bos_token, eos_token) {
        ...
        let bos_str = if tokenizer.special_token_id("<s>").is_ok() {
            "<s>"
        } else {
            "<|bos|>"
        };
        let eos_str = if tokenizer.special_token_id("</s>").is_ok() {
            "</s>"
        } else {
            "<|eos|>"
        };

        let ids_with_special = tokenizer.encode_with_special(text, Some(bos_str), Some(eos_str))?;
...
fn test_tokenizer_save_load_with_model() -> anyhow::Result<()> {
    ...
    // Create a tokenizer
    let corpus = ["hello world", "test message", "rust programming"];
    let tokenizer1 = Tokenizer::train_from_iterator(corpus.iter(), 500)?;

    // Create model with matching vocab_size
    let vocab_size = tokenizer1.vocab_size();
    let config = GPTConfig::with_tokenizer_vocab_size(vocab_size, 1024, 12, 6, 6, 768, None, None)?;
    let model = GPT::new(config);
