...
use nanochat_model::{attention::KVCache, GPTConfig, GPT};
...
fn test_forward_pass_basic() {
    ...
    let logits = model.forward_cache(&idx, None).expect("Failed to forward cache");
...
fn test_forward_pass_with_kv_cache() {
    ...
    let logits1 = model.forward_cache(&idx, Some(&mut kv_cache)).expect("Failed to forward cache");
    ...
    let logits2 = model.forward_cache(&idx, Some(&mut kv_cache)).expect("Failed to forward cache");
...
fn test_forward_pass_different_sequence_lengths() {
    ...
    let logits_short = model.forward_cache(&idx_short, None).expect("Failed to forward cache");
    assert_eq!(logits_short.shape(), &[1, 3, 50304]);

    // Test with longer sequence
    let idx_long = Tensor::zeros(&[1, 10]);
    let logits_long = model.forward_cache(&idx_long, None).expect("Failed to forward cache");
...
fn test_forward_pass_autoregressive() {
    ...
    let logits1 = model
        .forward_cache(&idx1, Some(&mut kv_cache))
        .expect("Failed to forward cache");
    assert_eq!(logits1.shape(), &[1, 1, 50304]);

    // Second token (reusing cache)
    let idx2 = Tensor::zeros(&[1, 1]);
    let logits2 = model
        .forward_cache(&idx2, Some(&mut kv_cache))
        .expect("Failed to forward cache");
    ...
    let logits3 = model
        .forward_cache(&idx3, Some(&mut kv_cache))
        .expect("Failed to forward cache");
