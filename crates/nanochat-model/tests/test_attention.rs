...
fn test_attention_basic() {
    ...
    let output = attn.forward(&x, None, 0, None).expect("Failed to forward attention");
...
fn test_attention_qk_norm() {
    ...
    let (q_norm, k_norm) = apply_qk_norm(&q, &k).expect("Failed to apply QK norm");
...
fn test_attention_kv_cache() {
    ...
    let output = attn
        .forward(&x, Some(&mut kv_cache), 0, None)
        .expect("Failed to forward attention");
...
fn test_attention_different_head_counts() {
    ...
    let out1 = attn1.forward(&x, None, 0, None).expect("Failed to forward attention");
    let out2 = attn2.forward(&x, None, 0, None).expect("Failed to forward attention");
    let out3 = attn3.forward(&x, None, 0, None).expect("Failed to forward attention");
