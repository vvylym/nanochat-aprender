...
- [ ] T036 Create `crates/nanochat-model/src/lib.rs` with public API exports
- [ ] T037 Implement `ModelConfig` struct in `crates/nanochat-model/src/config.rs` with validation (vocab_size, n_layer, n_head, n_kv_head, n_embd, sequence_len, block_size, dropout)
- [ ] T038 Implement `RMSNorm` function in `crates/nanochat-model/src/norm.rs` using aprender primitives (no learnable parameters)
